...
We iterate through the array using two nested loops to check every possible pair of numbers. 
For each pair, we check if their sum equals the target. If it does, we return the indices of those two numbers.

Time - O(n^2), because we are using two for loops and inner loop is nested. For each element, we try to find its second element by looping through the rest of the array
Space - O(1)
=========================
"""
